...
class BaumDiagramm(Scene):
    def construct(self):
        tree = BinomTree(
            num_events = 4, level_buff = 0,
            width = config["frame_width"]/2, height = config["frame_height"] - 2,
            line_config={"color": GREY, "stroke_width": 3}, 
            circle_config = {"color": BLACK, "fill_opacity": 1, "stroke_color": RED},
            dot_radius_buff = 0.04, prob_font_size = 24, cx_font_size = 36
        )
        tree.to_edge(LEFT)
        self.add(tree.get_pfad_nums())
        self.wait()

        self.play(DrawBorderThenFill(tree))
        self.wait()

        self.add(tree.get_c_numbers_next_to_tree())
        ...
        pfad_1 = tree.get_pfad([1, 4, 11, 25])
        pfad_1.set_color(YELLOW)
        self.play(Create(pfad_1), run_time = 2)
        self.wait()


        dot = Dot(point = tree.lines[0].get_start()).set_fill(opacity = 0)
        trace = TracedPath(dot.get_center, dissipating_time=0.75, stroke_opacity=[0, 1, 0], stroke_color = RED, stroke_width = 10)
        self.add(trace)
        self.play(
            MoveAlongPath(dot, pfad_1), run_time = 3
        )
        self.wait()


        # ################################################
        # # Potential: write as a method in BinomTreeScene
        # ################################################

        # pfad_list = [
        #     [1,5,13], [1,5,12], [1,4,11], [1,4, 10], 
        #     [0,3, 9], [0,3, 8], [0,2,7], [0,2,6]
        # ]

        # dots = VGroup(*[
        #     Dot(point = tree.lines[0].get_start()).set_fill(opacity = 0)
        #     for x in range(len(pfad_list))
        # ])
        # pfads = [
        #     tree.get_pfad(pfad_nums) 
        #     for pfad_nums in pfad_list
        # ]
        # traces = [
        #     TracedPath(dot.get_center, dissipating_time=0.75, stroke_opacity=[0, 1, 0], stroke_color = RED, stroke_width = 10)
        #     for dot in dots
        # ]
        # self.add(*traces)

        # self.play(
        #     *[MoveAlongPath(dot, pfad, run_time = 5) for dot, pfad in zip(dots, pfads)],
        # )
        # self.play(
        #     *[Create(pfad) for pfad in pfads],
        #     *[GrowFromCenter(circle) for circle in tree.circles],
        #     *[DrawBorderThenFill(mark) for mark in tree.cx_marks],
        #     run_time = 3
        # )
        # self.wait()

    def show_attributes_for_discord_maybe_useful_later(self):
        rect_dashed = DashedVMobject(
            Rectangle(width = tree.width, height = tree.height), 
            num_dashes = 100, color = GREY
        )
        rect_dashed.set_color(GREY)

        width_arrow = DoubleArrow(tree.get_left(), tree.get_right(), color = GREY, buff = 0).next_to(tree, DOWN)
        width_tex = Tex("width").add_background_rectangle(buff = 0.1).move_to(width_arrow)

        height_arrow = DoubleArrow(tree.get_bottom(), tree.get_top(), color = GREY, buff = 0).next_to(tree, LEFT, buff = 0.5)
        height_tex = Tex("height").add_background_rectangle(buff = 0.1).rotate(90*DEGREES).move_to(height_arrow)


        level_width = tree.level_width
        lines_dashed = VGroup()
        for r in range(4):
            line = DashedVMobject(Line(tree.get_bottom(), tree.get_top() + UP), num_dashes = 50)\
                .set_color(GREY)\
                .move_to(tree.get_left() + r*level_width * RIGHT + 0.5*UP)
            lines_dashed.add(line)

        level_width_arrow = DoubleArrow(
            lines_dashed[1].get_left(), lines_dashed[2].get_left(), color = GREY, buff = 0
        ).shift(0.5*DOWN)
        level_width_tex = Tex("level width", font_size = 24).move_to(level_width_arrow).shift(0.4*UP)

        num_levels = VGroup()
        for num_level in range(1, 5):
            tex = MathTex(str(num_level)).set_color(RED)
            num_levels.add(tex)
        num_levels.arrange_submobjects(RIGHT, buff = level_width).shift(3.5*UP)


        num_levels_tex = Tex("num levels").set_color(RED).next_to(num_levels, LEFT, buff = 1)

        
        self.add(num_levels, num_levels_tex)


        self.add(rect_dashed, width_arrow, width_tex, height_arrow, height_tex, lines_dashed[1:3], level_width_arrow, level_width_tex)
        self.add(tree.tree_lines, tree.tree_dots)
        self.wait()
